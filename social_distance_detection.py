...
if args['video']:
    ...
    cap = cv2.VideoCapture(0) # take input from camera
...
while cap.isOpened():
    ...
    for i in pos_dict.keys():
        for j in pos_dict.keys():
            if i < j:
                ...
                # Check if distance less than 2 metres or 2000 centimetres
                if dist < 200:
                    close_objects.add(i)
                    close_objects.add(j)
    ...
    #cv2.imwrite(str(fcount)+'.jpg',frame)
    fcount+=1
